<?php
/**
 * Mobile News SEO & OpenGraph Implementation
 */

function mobilenews_seo_meta_tags()
{
    // Basic SEO
    ...
    if (empty($image) && function_exists('mobilenews_get_option')) {
        $image = mobilenews_get_option('default_og_image');
    }

    // Fallback 2: Customizer OG Image
    if (empty($image) && function_exists('get_theme_mod')) {
        $image = get_theme_mod('mobilenews_default_og_image');
    }
    ...
    if (empty($image) && function_exists('mobilenews_get_option')) {
        $image = mobilenews_get_option('header_logo_url');
    }
...
add_action('wp_head', 'mobilenews_seo_meta_tags', 1);
