<?php
/**
 * Mobile News Ads Manager
 * Registers settings and fields for ad management.
 */

function mobilenews_ads_settings_init()
{
    // Check if Settings API is available
    if (!function_exists('is_admin') || !is_admin() || !function_exists('add_settings_section') || !function_exists('register_setting') || !function_exists('add_settings_field') || !function_exists('get_option')) {
        return;
    }

    register_setting('mobilenews_ads', 'mobilenews_ads_options');

    add_settings_section(
        'mobilenews_ads_section_header',
        'Header Ad (728x90)',
        'mobilenews_ads_section_header_cb',
        'mobilenews_ads'
    );

    add_settings_section(
        'mobilenews_ads_section_sidebar',
        'Sidebar Ad (300x250 or Responsive)',
        'mobilenews_ads_section_sidebar_cb',
        'mobilenews_ads'
    );

    add_settings_section(
        'mobilenews_ads_section_single',
        'Single Post Ads',
        'mobilenews_ads_section_single_cb',
        'mobilenews_ads'
    );

    add_settings_section(
        'mobilenews_ads_section_sticky',
        'Sticky Mobile Footer',
        'mobilenews_ads_section_sticky_cb',
        'mobilenews_ads'
    );

    // --- Header Fields ---
    add_settings_field(
        'header_ad_type',
        'Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_header',
        ['label_for' => 'header_ad_type']
    );
    add_settings_field(
        'header_ad_image',
        'Image URL',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_header',
        ['label_for' => 'header_ad_image']
    );
    add_settings_field(
        'header_ad_link',
        'Target URL',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_header',
        ['label_for' => 'header_ad_link']
    );
    add_settings_field(
        'header_ad_code',
        'Custom Code (HTML/JS)',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_header',
        ['label_for' => 'header_ad_code']
    );

    // --- Sidebar Fields ---
    add_settings_field(
        'sidebar_ad_type',
        'Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sidebar',
        ['label_for' => 'sidebar_ad_type']
    );
    add_settings_field(
        'sidebar_ad_image',
        'Image URL',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sidebar',
        ['label_for' => 'sidebar_ad_image']
    );
    add_settings_field(
        'sidebar_ad_link',
        'Target URL',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sidebar',
        ['label_for' => 'sidebar_ad_link']
    );
    add_settings_field(
        'sidebar_ad_code',
        'Custom Code (HTML/JS)',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sidebar',
        ['label_for' => 'sidebar_ad_code']
    );

    // --- Single Post Before Content Fields ---
    add_settings_field(
        'single_before_ad_type',
        'Before Content: Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_before_ad_type']
    );
    add_settings_field(
        'single_before_ad_image',
        'Before Content: Image',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_before_ad_image']
    );
    add_settings_field(
        'single_before_ad_link',
        'Before Content: Link',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_before_ad_link']
    );
    add_settings_field(
        'single_before_ad_code',
        'Before Content: Code',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_before_ad_code']
    );

    // --- Single Post In-Article Fields ---
    add_settings_field(
        'single_in_article_ad_paragraph',
        'In-Article: Paragraph Position',
        'mobilenews_ads_field_number_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_in_article_ad_paragraph']
    );
    add_settings_field(
        'single_in_article_ad_type',
        'In-Article: Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_in_article_ad_type']
    );
    add_settings_field(
        'single_in_article_ad_image',
        'In-Article: Image',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_in_article_ad_image']
    );
    add_settings_field(
        'single_in_article_ad_link',
        'In-Article: Link',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_in_article_ad_link']
    );
    add_settings_field(
        'single_in_article_ad_code',
        'In-Article: Code',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_in_article_ad_code']
    );

    // --- Single Post After Content Fields ---
    add_settings_field(
        'single_after_ad_type',
        'After Content: Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_after_ad_type']
    );
    add_settings_field(
        'single_after_ad_image',
        'After Content: Image',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_after_ad_image']
    );
    add_settings_field(
        'single_after_ad_link',
        'After Content: Link',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_after_ad_link']
    );
    add_settings_field(
        'single_after_ad_code',
        'After Content: Code',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_single',
        ['label_for' => 'single_after_ad_code']
    );

    // --- Sticky Mobile Footer Fields ---
    add_settings_field(
        'sticky_footer_ad_type',
        'Sticky Footer: Ad Type',
        'mobilenews_ads_field_type_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sticky',
        ['label_for' => 'sticky_footer_ad_type']
    );
    add_settings_field(
        'sticky_footer_ad_image',
        'Sticky Footer: Image',
        'mobilenews_ads_field_image_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sticky',
        ['label_for' => 'sticky_footer_ad_image']
    );
    add_settings_field(
        'sticky_footer_ad_link',
        'Sticky Footer: Link',
        'mobilenews_ads_field_link_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sticky',
        ['label_for' => 'sticky_footer_ad_link']
    );
    add_settings_field(
        'sticky_footer_ad_code',
        'Sticky Footer: Code',
        'mobilenews_ads_field_code_cb',
        'mobilenews_ads',
        'mobilenews_ads_section_sticky',
        ['label_for' => 'sticky_footer_ad_code']
    );
}
add_action('admin_init', 'mobilenews_ads_settings_init');

function mobilenews_ads_section_header_cb($args)
{
    echo '<p>Settings for the main header banner.</p>';
}
function mobilenews_ads_section_sidebar_cb($args)
{
    echo '<p>Settings for the sidebar widget area.</p>';
}
function mobilenews_ads_section_single_cb($args)
{
    echo '<p>Settings for ads within single articles.</p>';
}
function mobilenews_ads_section_sticky_cb($args)
{
    echo '<p>Display a fixed ad at the bottom of the screen (Mobile Only).</p>';
}

// --- Callbacks for Fields ---

function mobilenews_ads_field_type_cb($args)
{
    if (!function_exists('get_option')) {
        return;
    }
    $options = get_option('mobilenews_ads_options');
    $val = isset($options[$args['label_for']]) ? $options[$args['label_for']] : 'image';
    ?>
    <select name="mobilenews_ads_options[<?php echo esc_attr($args['label_for']); ?>]">
        <option value="image" <?php selected($val, 'image'); ?>>Image + Link</option>
        <option value="code" <?php selected($val, 'code'); ?>>Custom Code (AdSense/Script)</option>
    </select>
    <?php
}

function mobilenews_ads_field_image_cb($args)
{
    if (!function_exists('get_option')) {
        return;
    }
    $options = get_option('mobilenews_ads_options');
    $val = isset($options[$args['label_for']]) ? $options[$args['label_for']] : '';
    echo '<input type="text" name="mobilenews_ads_options[' . esc_attr($args['label_for']) . ']" value="' . esc_attr($val) . '" class="regular-text code" placeholder="https://example.com/banner.jpg">';
}

function mobilenews_ads_field_link_cb($args)
{
    if (!function_exists('get_option')) {
        return;
    }
    $options = get_option('mobilenews_ads_options');
    $val = isset($options[$args['label_for']]) ? $options[$args['label_for']] : '';
    echo '<input type="text" name="mobilenews_ads_options[' . esc_attr($args['label_for']) . ']" value="' . esc_attr($val) . '" class="regular-text code" placeholder="https://target-site.com">';
}

function mobilenews_ads_field_number_cb($args)
{
    if (!function_exists('get_option')) {
        return;
    }
    $options = get_option('mobilenews_ads_options');
    $val = isset($options[$args['label_for']]) ? $options[$args['label_for']] : '3';
    echo '<input type="number" name="mobilenews_ads_options[' . esc_attr($args['label_for']) . ']" value="' . esc_attr($val) . '" class="small-text" min="1">';
    echo '<p class="description">Insert ad after this many paragraphs.</p>';
}

function mobilenews_ads_field_code_cb($args)
{
    if (!function_exists('get_option')) {
        return;
    }
    $options = get_option('mobilenews_ads_options');
    $val = isset($options[$args['label_for']]) ? $options[$args['label_for']] : '';
    echo '<textarea name="mobilenews_ads_options[' . esc_attr($args['label_for']) . ']" rows="5" cols="50" class="large-text code">' . esc_textarea($val) . '</textarea>';
}

// --- Menu Page ---
// Integrated into Theme Options (inc/theme-options.php).

// --- Helper Function ---

function mobilenews_get_ad($slot_prefix)
{
    if (!function_exists('get_option')) {
        return '';
    }
    $options = get_option('mobilenews_ads_options');
    // $slot_prefix e.g., 'header_ad', 'sidebar_ad', 'single_before_ad'

    $type = isset($options[$slot_prefix . '_type']) ? $options[$slot_prefix . '_type'] : 'image';
    $output = '';

    if ($type === 'image') {
        $img = isset($options[$slot_prefix . '_image']) ? $options[$slot_prefix . '_image'] : '';
        $link = isset($options[$slot_prefix . '_link']) ? $options[$slot_prefix . '_link'] : '#';

        // Dummy Ad Logic
        if (empty($img)) {
            $size = '728x90';
            $text = 'Advertisement';

            switch ($slot_prefix) {
                case 'header_ad':
                    $size = '728x90';
                    $text = 'Header Ad (728x90)';
                    break;
                case 'sidebar_ad':
                    $size = '300x250';
                    $text = 'Sidebar Ad (300x250)';
                    break;
                case 'single_before_ad':
                    $size = '728x90';
                    $text = 'Before Content Ad';
                    break;
                case 'single_in_article_ad':
                    $size = '300x250';
                    $text = 'In-Article Ad';
                    break;
                case 'single_after_ad':
                    $size = '728x90';
                    $text = 'After Content Ad';
                    break;
                case 'sticky_footer_ad':
                    $size = '320x50';
                    $text = 'Sticky Footer';
                    break;
            }
            $img = 'https://placehold.co/' . $size . '?text=' . urlencode($text);
        }

        if (!empty($img)) {
            $output .= '<div class="modernnews-ad-container modernnews-ad-image">';
            $output .= '<a href="' . esc_url($link) . '" target="_blank" rel="nofollow noopener">';
            $output .= '<img src="' . esc_url($img) . '" alt="Advertisement" class="w-full h-auto object-cover rounded-lg mx-auto">';
            $output .= '</a>';
            $output .= '</div>';
        }
    } elseif ($type === 'code') {
        $code = isset($options[$slot_prefix . '_code']) ? $options[$slot_prefix . '_code'] : '';
        if (!empty($code)) {
            $output .= '<div class="modernnews-ad-container modernnews-ad-code">';
            $output .= $code; // Intentionally unescaped for Scripts
            $output .= '</div>';
        }
    }

    return $output;
}

/**
 * Filter Content to Inject Ad
 */
// function mobilenews_inject_in_article_ad($content)
// {
//    // Replaced by inc/ad-injection.php
// }
// add_filter('the_content', 'mobilenews_inject_in_article_ad');
