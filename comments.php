    ...
    <?php if (have_comments()): ?>
        <div class="flex items-center gap-3 mb-10">
            ...
            <h2 class="text-2xl font-black">
                <?php
                $mobilenews_comment_count = get_comments_number();
                if ('1' === $mobilenews_comment_count) {
                    printf(
                        /* translators: 1: title. */
                        esc_html__('Satu Komentar di &ldquo;%1$s&rdquo;', 'mobilenews'),
                        '<span>' . get_the_title() . '</span>'
                    );
                } else {
                    printf(
                        /* translators: 1: comment count number, 2: title. */
                        esc_html(_nx('%1$s Komentar', '%1$s Komentar', $mobilenews_comment_count, 'comments title', 'mobilenews')),
                        number_format_i18n($mobilenews_comment_count),
                        '<span>' . get_the_title() . '</span>'
                    );
                }
                ?>
        ...
        the_comments_navigation(array(
            'prev_text' => '<span class="flex items-center gap-2"><i class="ri-arrow-left-s-line"></i> ' . esc_html__('Komentar Lama', 'mobilenews') . '</span>',
            'next_text' => '<span class="flex items-center gap-2">' . esc_html__('Komentar Baru', 'mobilenews') . ' <i class="ri-arrow-right-s-line"></i></span>',
        ));
        ...
        if (!comments_open()):
            ...
            <div
                ...
                <?php esc_html_e('Kolom komentar telah ditutup.', 'mobilenews'); ?>
            </div>
            <?php
        endif;
